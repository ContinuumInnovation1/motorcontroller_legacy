...
{ 
    public:
        ...
        void HandleCanMessage(const SimpleCanRxHeader rxHeader, const uint8_t *rxData)
        {
            ...
            if (ReceivedID!=-1){
                int msg_id=IDHandler(ReceivedID);
	            if(msg_id>=0){
                    if(CAN_ID==0&&msg_id<count_motor_values&&ReceivedID/message_space<max_device_count){
                        motor_values[ReceivedID/message_space][msg_id]=Value;
                        //Serial.printf("Recived message withID:%d , value:%f\n",ReceivedID,Value);
                    }
                    if(CAN_ID>0&&msg_id<count_target_parameter){
                        target_values[ReceivedID/message_space][msg_id]=Value;
                        Serial.printf("Recived message withID:%d , value:%f\n",ReceivedID,Value);
                        if(msg_id==count_target_parameter-1){
                            full_instruction_recived=true;
                        }  
                    }
	            }
